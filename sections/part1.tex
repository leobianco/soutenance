    ...
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item[{\color{white}\ding{118}}] {\color{white}\textit{Community detection}: group similar nodes, sensitive to \textit{outliers}}
        ...
        \begin{column}{0.5\linewidth}
            ...
            \begin{figure}
                ...
                \caption*{\scriptsize \color{gray} Jazz collaborations in New York, Chicago, and elsewhere~\footnotemark[1]}
            \end{figure}
        \end{minipage}
        \end{column}
    \end{columns}
    \grayfootnotetext[1]{\citet{gleiser2003community}}
\end{frame}
...
\begin{frame}{Motivation}
    ...
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item \textit{Adjacency matrix}: symmetric \(A \in {\{0, 1\}}^{n \times n}\)
                \item \textit{Community detection}:~\footnotemark[2] group similar nodes, sensitive to \textit{outliers}
                \item[{\color{white}\ding{118}}] {\color{white}\textit{Robust algorithm}: accurate results despite outliers}
            \end{itemize}
        \end{column}
        \begin{column}{0.5\linewidth}
        ...
        \end{column}
    \end{columns}
    \grayfootnotetext[2]{\citet{abbe2023communitydetectionstochasticblock}}
\end{frame}
...
% \begin{frame}{The Stochastic Block Model~\textsuperscript{3}}
%     \begin{columns}
%         \column{0.5\linewidth}
...
%     \end{columns}
%     \grayfootnotetext[3]{\citet{HOLLAND1983109}}
% \end{frame}
...
\begin{frame}{The Stochastic Block Model~\textsuperscript{3}}
    \begin{columns}
        \column{0.5\linewidth}
         \uncover<1->{\(Z_i \to \text{community of node } i\) \\[1em]
            \(K \; \to \text{nb. of communities}\) \\[1em]
            \(\textcolor{black}{\pi_{k}} \to \text{size of community } k\) \\[1em]
            \(\textcolor{myorange}{\Gamma_{kl}} \to \text{connectivity } k, l\) \\[1.5em]
            }
           \uncover<2->{\((Z, A) \sim \textup{SBM}_K (\pi, \Gamma)\) 
            \begin{equation*}
                \begin{cases}
                    \mathbb{P}(Z_i = k) = \pi_k \\
                    \mathbb{P}(A_{i j} = 1 \vert Z_i = k, Z_j = l) = \Gamma_{kl}
...
\begin{frame}{The Corrupted Stochastic Block Model~\textsuperscript{4}}
    \begin{columns}
        \column{0.5\linewidth}
        ...
        \column{0.5\linewidth}
            ...
            \end{minipage}
    \end{columns}
    \grayfootnotetext[4]{\citet{liu2022minimax}}
\end{frame}
% --- END OF METHODOLOGY ---
...
\begin{frame}{Results}
    \begin{itemize}
        \item Idea: find subgraph \(S=S_1 \cup \dots \cup S_K\) excluding outliers
        \begin{equation*}
            \Rightarrow \quad \hat{\Gamma}_{k l} = \left(\sum_{i \in S_k j \in S_l} A_{i j}\right) / \vert S_k \vert \vert S_l \vert \quad \textup{is a good estimator}
        \end{equation*}
        \pause
        \item \textbf{Contribution \#1}: extend error bound in~\citet{pmlr-v178-acharya22a} to \(K > 1\)
    \end{itemize}
    \begin{tcolorbox}[colback=yellow!45!white,
                    colframe=yellow!45!white]
        \textbf{Theorem~\citep{bianco2025subsearchrobustestimationoutlier}.} Let \(S = S_1 \cup \dots \cup S_K\) be a subgraph, \(\Omega_k\) the nodes in community \(k\), \(\mathcal{I}\) the inliers.
        Let
        \(\hat{\Gamma}_{k l} = \left(\sum_{i \in S_k j \in S_l} A_{i j}\right) / \vert S_k \vert \vert S_l \vert\) and \(\hat{Q}(S)_{i j} = \hat{\Gamma}_{S(i) S(j)}\).
        Then,
        \begin{align*}
...
\begin{frame}{Results}
    ...
    \begin{itemize}
        ...
        \item \textbf{Contribution \#1}: extend error bound in~\citet{pmlr-v178-acharya22a} to \(K > 1\)
        \item \textbf{Contribution \#2}: \textsc{SubSearch} algorithm, finding \(S\) by optimizing \(c(S) \coloneqq \lVert A_S - \hat{Q}(S) \rVert_{\textup{op}}\) via Simulated Annealing
        \item \textbf{Code}: {\color{myblue}\url{github.com/leobianco/robust_estim_sbm}}
    \end{itemize}
\end{frame}
...
\begin{frame}{Discussion for Robust Community Detection}
    \begin{itemize}
