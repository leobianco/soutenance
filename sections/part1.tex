...
\begin{frame}{Motivation}
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item \textit{Community detection}: group similar nodes in a graph.
                \item Algorithms can be \textbf{highly sensitive} to \textit{outlier} nodes.
                \item[{\color{white}\ding{118}}] {\color{white}\textbf{Goal}: achieve ``good'' results despite outliers.}
            \end{itemize}
        \end{column}
        \begin{column}{0.5\linewidth}
            \begin{figure}
                ...
                \caption*{\scriptsize \color{gray} Graph of Jazz collaborations in New York, Chicago, and Other Cities~\citep{gleiser2003community}.}
            \end{figure}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{figure}
                ...
                \caption*{\scriptsize \color{gray}Spectral clustering applied to the graph of Jazz collaborations~\citep{gleiser2003community}.}
            \end{figure}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item \textit{Community detection}: group similar nodes in a graph.
                \item Algorithms can be \textbf{highly sensitive} to \textit{outlier} nodes.
                \item {\color{green!60!black}\textbf{Goal}}: achieve ``good'' results despite outliers.
            \end{itemize}
        \end{column}
        \begin{column}{0.5\linewidth}
            \begin{figure}
...
\begin{frame}{The Stochastic Block Model~\citep{HOLLAND1983109}}
    \begin{columns}
        \column{0.5\linewidth}
            \textit{Probabilistic model}
            \\[1em]
            \hspace{1em} \(Z_i \to \text{community of node } i\) \\[1em]
            \hspace{1em} \(K \; \to \text{nb. of communities}\) \\[1em]
            \hspace{1em} \(\textcolor{myblue}{\pi_{k}} \to \text{size of community } k\) \\[1em]
...
\begin{frame}{The Stochastic Block Model~\citep{HOLLAND1983109}}
    ...
    \begin{columns}
        \column{0.5\linewidth}
            ...
            \hspace{1em} \(\textcolor{myblue}{\pi_{k}} \to \text{size of community } k\) \\[1em]
            \hspace{1em} \(\textcolor{myorange}{\Gamma_{kl}} \to \text{connectivity } k, l\) \\[1.5em]
            \((Z, A) \sim \textup{SBM}_{K} (\textcolor{myblue}{\pi}, \textcolor{myorange}{\Gamma})\) 
            \begin{equation*}
                \begin{cases}
                    \mathbb{P}(Z_i = k) = \textcolor{myblue}{\pi_k} \\
...
\begin{frame}{The Corrupted Stochastic Block Model}
    \begin{columns}
        \column{0.5\linewidth}
            {\color{myred}\textbf{Adversary}} introduces outliers:
            \vspace{0.75em}
            \begin{enumerate}
                \item \((Z, A_{\textup{pure}}) \sim \textup{SBM}_{K}(\textcolor{myblue}{\pi}, \textcolor{myorange}{\Gamma})\)
                \item Adversary \textit{arbitrarily} changes edges of \(\gamma n\) nodes
...
\begin{frame}{Research question}
    \begin{itemize}
        ...
        % \end{equation*}
        \item \underline{Task}: robustly estimate the connectivities \(\textcolor{myorange}{\Gamma}\).
        \item For \(K=1\), \citet{pmlr-v178-acharya22a}.
    \end{itemize}
    % \pause
...
\begin{frame}{Results}
    \begin{itemize}
        \item \textit{Intuition:} find subgraph \(S\) excluding worst outliers.
        \item \textbf{First contribution}: extended bound from~\citet{pmlr-v178-acharya22a} to \(K > 1\).
    \end{itemize}
    \pause
    \begin{tcolorbox}[colback=yellow!45!white,
                    colframe=yellow!45!white]
        \textbf{Theorem.} Denote \(S\) a subgraph clustered into \(S_1, \dotsc, S_K\), \(\Omega_k\) community \(k\), \(\mathcal{I}\) the set of inlier nodes.
        Let
...
\begin{frame}{Results}
    ...
    \begin{itemize}
        \item \textit{Intuition:} find subgraph \(S\) excluding worst outliers.
        \item \textbf{First contribution}: extended bound from~\citet{pmlr-v178-acharya22a} to \(K > 1\).
        \item \textbf{Second contribution} (\textsc{SubSearch}): finding \(S\) by optimizing \(c(S) \coloneqq \lVert A_S - \hat{Q}(S) \rVert_{\textup{op}}\) via Simulated Annealing.
        \item \textbf{Code}: \url{github.com/leobianco/robust_estim_sbm}
    \end{itemize}
\end{frame}
...
{\color{green!60!black}\textbf{Goal}}: explore the space \(\mathcal{S}\) of subgraphs \(S \subset G\) of size \((1-\gamma)n\),
...
\begin{itemize}
    \uncover<2->{%
        \item[\color{blue!70}\large\ding{172}] \textbf{Initialize}
        at a high temperature \(T_0\), random subgraph \(S_0\)
    }

    \uncover<3->{%
        \item[\color{blue!70}\large\ding{173}] \textbf{Propose}
        \(S_{\text{candidate}}\): swap 
        \(i \in S_{\text{current}}\) with \(j \notin S_{\text{current}}\)
    }

    \uncover<4->{%
        \item[\color{blue!70}\large\ding{174}] \textbf{Accept or reject:}
        compute \(\Delta = c(S_{\text{current}}) - c(S_{\text{candidate}})\),\\
        accept with probability \(\min\!\left(1, \exp(\Delta / T_t)\right)\)
    }

    \uncover<5->{%
        \item[\color{blue!70}\large\ding{175}] \textbf{Cool down}
        by \(T_{t+1} = c\,T_t\), \(c \approx 1\)
    }
    \uncover<6->{}
\end{itemize}
...
\begin{frame}{Results}
    ...
    \begin{figure}
        ...
        \begin{subfigure}{\textwidth}
            \centering
            \includegraphics[height=0.5\textheight]{figures/part1/simulated_annealing_true_outliers.png}
...
\begin{frame}{Results}
    ...
    \begin{figure}
        ...
        \begin{subfigure}{\textwidth}
            \centering
            \includegraphics[height=0.5\textheight]{figures/part1/filtering_true_outliers.png}
...
\begin{frame}{Discussion}
    \begin{itemize}
        \item \textbf{Main take away}: ``exploring'' the space of subgraphs \(\Rightarrow\) find subgraphs avoiding outliers.
        \item \underline{Limitation \# 1}: can we rigorously prove robustness?
        \item \underline{Limitation \# 2}: can we provide faster rates?
    \end{itemize}
