...
\begin{minipage}[t][10em][t]{\textwidth}
    ...
    \only<4-5>{
        \addtocounter{framenumber}{1}
        \textcolor{myorange}{\textit{Alignment}} to human preferences via Reinforcement Learning~\citep{christiano2023deepreinforcementlearninghuman}:\\[-0.25em]
        \begin{enumerate}
...
\begin{minipage}[t][10em][t]{\textwidth}
    \only<1->{\textbf{Problem}: \(\mathcal{D}_{\textup{RM}}\) is costly, time-consuming, and error-prone to get}
    \\[1em]
    {\only<2->{
...
\begin{frame}{Creating Synthetic Hallucinations}
    NPOV dataset~\citep{chang2024detectinghallucinationcoverageerrors}:
\begin{table}[h]
    \centering
    \begin{tabular}{lcccc}
        \hline
