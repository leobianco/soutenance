...
\begin{minipage}[t][10em][t]{\textwidth}
    ...
    \only<4-5>{
        ...
        \begin{enumerate}
            \item<4-5> Train a reward model \(\!R\) on \(\mathcal{D}_{\textup{RM}}\)
        ...
        \begin{equation*}
            \pi_{\beta}
            \in \arg\max_{\pi}\,
            \mathbb{E}_{\substack{x \sim \mathcal{D}_{\textup{RL}} \\ y \sim p(y\mid x)}}\!\left[R(x, y)\right]
            - \beta\,\mathrm{KL}\!\left(\pi \,\Vert\, \pi_{\textup{SFT}}\right)
        \end{equation*}
        }
    }
    ...
    \only<6>{
        \vspace{1em}
        \textit{Parameter-efficient tuning} \(\Rightarrow\) Low-Rank Adaptation (LoRA)~\citep{hu2021loralowrankadaptationlarge}:
        \begin{equation*}
            \theta = \theta_{\textup{SFT}} + A B
        \end{equation*}
    }
